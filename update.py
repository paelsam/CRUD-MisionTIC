        ...
        if i[0].value == identificador:
            ...
            for d in datos_actualizados:
                ...
                if d == 'Titulo' and not(datos_actualizados[d] == ''):
                    ...
                    hoja.cell(
                        row=fila, colum=titulo).value = datos_actualizados[d]
                elif d == 'Descripcion' and not(datos_actualizados[d] == ''):
                    hoja.cell(
                        row=fila, colum=descripcion).value = datos_actualizados[d]
                elif d == 'Estado' and not(datos_actualizados[d] == ''):
                    hoja.cell(
                        row=fila, colum=estado).value = datos_actualizados[d]
                elif d == 'Fecha Inicio' and not(datos_actualizados[d] == ''):
                    hoja.cell(
                        row=fila, colum=fecha_inicio).value = datos_actualizados[d]
                elif d == 'Fecha Finalizado' and not(datos_actualizados[d] == ''):
                    hoja.cell(
                        row=fila, colum=fecha_finalizado).value = datos_actualizados[d]
    # Guardamos el cambio con esta duncion
...
valores_actualizar = {
    "Titulo": "loading...",
    "Descripcion": "Nashee",
    "Estado": "In process pa",
    "Fecha Inicio": "Hoy",
    "Fecha Finalizado": "Ayer"
}
actualizar()
